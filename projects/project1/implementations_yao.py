...
def mean_squared_error_gd(y, tx, initial_w, max_iters, gamma):
    ...
    def compute_loss(y, tx, w):
        data_num = y.shape[0]
        e_mat = y - np.dot(tx, w)
        return 0.5 * np.dot(e_mat.T, e_mat) / data_num
...
def logistic_regression(y, tx, initial_w, max_iters, gamma, sgd=True):
    ...
    def compute_loss(y, tx, w):
        ...
        loss = -np.dot(y, np.log(g + eps)) - np.dot((1 - y).T, np.log(1 - g + eps))
        return loss

    def compute_gradient(y, tx, w):
        data_num = y.shape[0]
        g = sigmoid(np.dot(tx, w))
        grad = np.dot(tx.T, g - y)
        return grad
...
def reg_logistic_regression(y, tx, lambda_, initial_w, max_iters, gamma, sgd=True):
    """Logistic regression for classification.
    ...
    def compute_gradient(y, tx, w, lambda_):
        data_num = y.shape[0]
        g = sigmoid(np.dot(tx, w))
        grad = np.dot(tx.T, g - y) + 2.0 * lambda_ * w
        return grad
