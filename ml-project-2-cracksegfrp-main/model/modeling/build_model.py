...
def set_ss_model(cfg):
    ...
    if cfg.MODEL.DETECTOR_TYPE == 'u-net16':
        segmentation_model = UNet16(num_classes=num_classes, pretrained=True, up_sampling_method=cfg.MODEL.UP_SAMPLE_METHOD, rmp=cfg.MODEL.RMP, re_method=cfg.MODEL.RE_METHOD, receptive_enlarge=cfg.MODEL.RECEPTIVE_ENLARGE, Residual=cfg.MODEL.RESIDUAL, gamma_HFE=cfg.MODEL.GAMMA_HFE)
...
def set_ss_loss(num_train_ds, resume_iter, cfg):
    if cfg.SOLVER.SEG_LOSS_FUNC == "BCE":
        ...
        ss_loss_fn = BoundaryComboLoss(per_epoch, pos_weight=pos_weight, loss_weight=loss_weight, decrease_ratio=cfg.SOLVER.BOUNDARY_DEC_RATIO, sce_weight = cfg.SOLVER.SCE_WEIGHT, sce = cfg.SOLVER.SCE).to('cuda')
        ...
        loss_weight = cfg.SOLVER.WB_AND_D_WEIGHT
        per_epoch = num_train_ds // cfg.SOLVER.BATCH_SIZE + 1
        ss_loss_fn = GeneralizedBoundaryComboLoss(per_epoch, pos_weight=pos_weight, loss_weight=loss_weight, sce_weight = cfg.SOLVER.SCE_WEIGHT, sce = cfg.SOLVER.SCE).to('cuda')
    else:
