...
class BoundaryComboLoss(nn.Module):  
    def __init__(self, per_epoch, smooth=10 ** -8, reduction='none', pos_weight=[1, 1], loss_weight=[1, 1], alpha_min=0.01, decrease_ratio=1, sce_weight = 0.1,  sce = False):
        ...
        self.wbce_dice_loss = BCE_DiceLoss(smooth=smooth, pos_weight=pos_weight, loss_weight=loss_weight, sce_weight = sce_weight, sce = sce).to('cuda') 
...
class GeneralizedBoundaryComboLoss(nn.Module):  
    def __init__(self, per_epoch, smooth=10 ** -8, reduction='none', pos_weight=[1, 1], loss_weight=[1, 1], alpha_min=0.01, decrease_ratio=1, sce_weight = 0.1, sce = False):
        ...
        self.wbce_dice_loss = BCE_DiceLoss(smooth=smooth, pos_weight=pos_weight, loss_weight=loss_weight, gdice=True, sce_weight = sce_weight,  sce = sce).to('cuda') 
        self.bd_loss = BoundaryLoss().to('cuda')
...
class WeightedBCELoss(nn.Module):
    def __init__(self, smooth=10 ** -8, reduction='mean', pos_weight=[1, 1], sce_weight = 0.1, sce = False):
        ...
        self.sce_weight = sce_weight
        if(self.sce):
            print("SCE WEIGHT: ", self.sce_weight)
    ...
    def forward(self, predict, target):
        ...
        if(self.sce):
            loss_sce = - (self.pos_weight[0]*predict*torch.log(torch.clamp(target, 1e-4, 1.0)) + self.pos_weight[1]*(1-predict)*torch.log(torch.clamp((1-target), 1e-4, 1.0)))/sum(self.pos_weight)
            loss = (1 - self.sce_weight) * loss + self.sce_weight * loss_sce
        if self.reduction == 'mean':
...
class BCE_DiceLoss(nn.Module):
    ...
    def __init__(self, smooth=1, p=2, reduction='none', pos_weight=[1, 1], loss_weight = [1, 1], gdice=False, sce_weight = 0.1, sce = False):
        super(BCE_DiceLoss, self).__init__()
        self.smooth = smooth
        self.p = p
        self.reduction = reduction
        self.sce_weight = sce_weight
        self.bce_loss = WeightedBCELoss(pos_weight=pos_weight, sce_weight = sce_weight, sce = sce).to('cuda')
        if gdice:
