...
def mosaic(img_list, mask_list):
    ...
    mosaic_img = torch.zeros((c,h,w))
    mosaic_img_mask = torch.zeros((1,h,w))
    ...
    return mosaic_img[None, ...], mosaic_img_mask[None, ...]
...
def do_train(args, cfg, model, optimizer, scheduler, train_loader, eval_loader):
    ...
    for iteration, (imgs, sr_targets, segment_targets) in enumerate(train_loader, args.resume_iter+1):
        if(args.mosaic):
            jd = random.randint(0, 1)
            if(jd == 1):
                add_img, add_mask = None, None
                for mosaic_step in range(6):
                    choose_idx = np.random.choice(len(sr_targets), size=4, replace = False)
                    mosaic_img, mosaic_mask = mosaic(sr_targets[choose_idx], segment_targets[choose_idx])
                    if(add_img is None):
                        add_img = mosaic_img
                        add_mask = mosaic_mask
                    else:
                        add_img = torch.cat([add_img, mosaic_img], dim = 0)
                        add_mask = torch.cat([add_mask, mosaic_mask], dim = 0)
                sr_targets = torch.cat([sr_targets, add_img], dim = 0)
                segment_targets = torch.cat([segment_targets, add_mask], dim = 0)
