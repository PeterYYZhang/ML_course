...
import pandas as pd
import wandb
from model.utils.estimate_metrics import PSNR, SSIM, IoU, Acc, Rec
...
def inference_for_ss(args, cfg, model, test_loader):
    ...
    for iteration, (imgs, sr_targets, masks, fname) in enumerate(test_loader, 1):
        ...
        # sr_preds, segment_preds = model(sr_targets)
#         segment_preds_detach = segment_preds[0].detach().cpu()
#         segment_preds_pil = transforms.ToPILImage()(segment_preds_detach)
#         segment_preds_pil.save(os.path.join('./datasets/masks/', fname[0]))
        
#         segment_preds_detach = sr_targets[0].detach().cpu()
#         segment_preds_pil = transforms.ToPILImage()(segment_preds_detach)
#         segment_preds_pil.save(os.path.join('./datasets/images/', "ori"+fname[0]))
        
#         segment_preds_detach = masks[0].detach().cpu()
#         segment_preds_pil = transforms.ToPILImage()(segment_preds_detach)
#         segment_preds_pil.save(os.path.join('./datasets/gt_masks/', "gt"+fname[0]))
        ...
        for iou_th in thresholds:
            ...
            if 'acc_scores' in locals():
                acc_scores = np.append(acc_scores, acc(segment_preds_bi, masks.to("cuda"))[:, np.newaxis], axis=1)
            else:
                acc_scores = np.copy(acc(segment_preds_bi, masks.to("cuda"))[:, np.newaxis])
            if 'rec_scores' in locals():
                rec_scores = np.append(rec_scores, rec(segment_preds_bi, masks.to("cuda"))[:, np.newaxis], axis=1)
            else:
                rec_scores = np.copy(rec(segment_preds_bi, masks.to("cuda"))[:, np.newaxis])

        if aiu_scores.size != 0:
            aiu_scores = np.append(aiu_scores, iou_scores, axis=0)
        else:
            aiu_scores = np.copy(iou_scores)
        if aac_scores.size != 0:
            aac_scores = np.append(aac_scores, acc_scores, axis=0)
        else:
            aac_scores = np.copy(acc_scores)
        if are_scores.size != 0:
            are_scores = np.append(are_scores, rec_scores, axis=0)
        else:
            are_scores = np.copy(rec_scores)
        if args.wandb_flag:
        # wandb
        ...
        if iteration % 10 == 0:
            ...
            print(f"PSNR_mean:{np.mean(psnr_scores):.4f}  SSIM_mean:{np.mean(ssim_scores):.4f} {iou_mode}_mean:{np.mean(aiu_scores):.4f} {acc_mode}_mean:{np.mean(aac_scores):.4f} {rec_mode}_mean:{np.mean(are_scores):.4f}")
    save_file = f"./datasets/{cfg.OUTPUT_DIR.split('/')[-3]}.txt"
    iou_diff_thres = np.mean(aiu_scores, axis = 0)
    best_iou_idx = np.argmax(iou_diff_thres)
    iou_th_best = thresholds[best_iou_idx]
    best_iou = iou_diff_thres[best_iou_idx]
    acc_diff_thres = np.mean(aac_scores, axis = 0)
    best_acc_idx = np.argmax(acc_diff_thres)
    acc_th_best = thresholds[best_acc_idx]
    best_acc = acc_diff_thres[best_acc_idx]
    rec_diff_thres = np.mean(are_scores, axis = 0)
    best_rec_idx = np.argmax(rec_diff_thres)
    rec_th_best = thresholds[best_rec_idx]
    best_rec = rec_diff_thres[best_rec_idx]
    with open(save_file, 'w') as f:
        # f.write(f"{np.mean(aiu_scores):.4f}, {best_iou}, {iou_th_best}")
        f.write(f"{np.mean(aac_scores):.4f}, {best_acc}, {acc_th_best}")
        # f.write(f"{np.mean(are_scores):.4f}, {best_rec}, {rec_th_best}")
